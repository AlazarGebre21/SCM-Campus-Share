...
const Bookmarks = () => {
  ...
  // Helper to remove item from UI immediately
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {bookmarks.map((bookmark) => (
            <ResourceCard
              key={bookmark.id}
              // PASS THE BOOKMARK ID HERE
              resource={{
                ...bookmark.resource,
                is_bookmarked: true,
                bookmark_id: bookmark.id,
              }}
              // Pass callback to remove card from screen when unbookmarked
              onUnbookmark={() => handleRemove(bookmark.id)}
            />
          ))}
        </div>
      )}
